...
│   │   └── TodoList/                     # ToDoリスト機能に関連するコンポーネント
│   │       ├── TodoList.vue                 # コンポーネント本体
│   │       ├── TodoList.stories.ts          # Storybook用のストーリーファイル
│   │       └── TodoList.spec.ts             # スナップショットテスト、コンポーネントのテスト
│   └── ui/                               # オプショナル: UIフレームワークを使う場合は削除可
│       └── Button/                       # ボタンコンポーネント
│           ├── Button.vue                 # コンポーネント本体
│           ├── Button.stories.ts          # Storybook用のストーリーファイル
│           └── Button.spec.ts             # スナップショットテスト、コンポーネントのテスト
...
- パッケージやコンポーネントの import 元
- 権限
- OS やパッケージやソフトのバージョン
- github issue に上がっているような頻出の問題
- データベースのレコード
- DB 以外のデータ
- SaaS や外部システムの状態
- ネットワーク
